...
 }
/*
function y(x, t, A, T, lambda){
  ytest = A * cos(TWO_PI * (t/T - x/lambda));
  print(ytest);

  return A * cos(TWO_PI * (t/T - x/lambda));
}
*/
function draw(){
...
function keyPressed(){
  if (keyCode == 32){ // barre espace
    arret();
  }else if (keyCode == 38 && lambda>0.1*width){ // fleche haut
    lambda -= 10;
  }else if (keyCode == 40 && lambda<1.8*width){ // fleche bas
    lambda += 10;
  }else if (keyCode == 37 && T<10){ // fleche gauche
    T += 0.1;
  }else if (keyCode == 39 && T>0.2){ // fleche droite
    T -= 0.1;
